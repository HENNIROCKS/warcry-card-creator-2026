...
**Deployment card editor** — build a deployment card with up to 4 player colours (red, blue, green, yellow). Tap any snap position on the card to open a popover with options: place a marker (dagger, shield, or hammer icon with an optional round label), add a measurement line (tap a second position to complete the line; set start/end caps and a text label in the popover), place an objective marker (black circle with a text label such as "The Tower"), draw a rectangular zone (tap a start corner then an end corner to shade an area in the player's colour), or place a mask circle (single tap; S/M size; renders in the battlefield background colour to carve a circular exclusion through any zones beneath). Mask circle edges expose 8 perimeter snap points that can be used as measurement endpoints. Tap any existing marker, objective, or zone to edit or remove it. The View dropdown independently controls Show Markers, Show Runemarks (falls back to letter initials when off), Orientation runemark (top-left corner), and Matched Play runemark (top-right corner). Save and load card layouts as JSON. Pinch-to-zoom and pan the card on mobile; double-tap resets to fit. Export as a print-ready PNG; printer-friendly mode produces a strictly black-and-white card with per-player SVG hatch patterns on zones and numbered player badges.
