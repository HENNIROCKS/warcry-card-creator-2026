...
**Deployment card editor** — build a deployment card with up to 4 player colours (red, blue, green, yellow). Tap any snap position on the card to place a deployment point (dagger, shield, or hammer icon with an optional round label); tap an occupied position to edit or remove it. Add measurement lines in two taps: tap a position and choose "Measurement", then tap a second position to place the line; the measurement popover lets you set start/end caps (dot, arrow, tick, or none) and a text label. Toggle the Show Markers switch to show or hide all interactive dots. Save and load card layouts as JSON. Pinch-to-zoom and pan the card on mobile; double-tap resets to fit. Export as a print-ready PNG; printer-friendly mode produces a strictly black-and-white card with numbered player badges.
