...
**Deployment card editor** — build a deployment card with up to 4 player colours (red, blue, green, yellow). Tap any snap position on the card itself to place a deployment point (dagger, shield, or hammer icon with an optional round label); tap again to remove it. Toggle the Overlays switch to show or hide all interactive dots and measurement handles on the preview. Add measurement lines by clicking a snap position in measurement mode, then picking a direction from the 8-way arrow picker that appears on the card. Save and load card layouts as JSON. Pinch-to-zoom and pan the card on mobile; double-tap resets to fit. Export as a print-ready PNG; printer-friendly mode produces a strictly black-and-white card with numbered player badges.
