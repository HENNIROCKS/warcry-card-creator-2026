...
<script lang="ts">
	...
	function makeSlug() {
		...
		const parts = [data.grandAlliance, data.faction, data.bladeborn]
			.filter(Boolean)
			.map(toSlug);
		parts.push(toSlug(data.name || 'fighter'));
		const now = new Date();
	...
	function saveLayout() {
		showDropdown = false;
		const { modelImage, ...rest } = data;
		const payload = {
			...rest,
			rightRunemarks: rest.rightRunemarks.map(({ id, label }) => ({ id, label }))
		};
